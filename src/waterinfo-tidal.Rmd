...
```{r echo=FALSE}
eel <- read_csv("../data/eel_track_buttercup.csv", col_types = cols())
```
...
ggplot(eel, aes(x = Arrival, xend = Departure, 
                 y = distance_to_refence_station, 
                 yend = distance_to_refence_station,
                 color = eel_name)) +
    geom_segment(size = 3) + xlab("") + ylab("") +
    geom_line(aes(x = Arrival, y = distance_to_refence_station), size = 1) +
