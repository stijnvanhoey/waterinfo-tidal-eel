...
eel %>% 
  ...
  addCircleMarkers(~longitude, ~latitude,
                   popup = ~receiver,
                   color = "#e66101", radius = ~total_residence/3600, 
...
municipalities <-
  ...
  mutate(municipality = case_when(
    ...
    station == "ws-DL7" ~ "Reference station"
  )) %>%
...
# Create plot
ggplot(eel_gather,
  aes(x = datetime, y = distance_from_sea, color = eel_name)) +
  xlab("") +
  ylab("") +
  geom_line(alpha = 1., color = "#e66101", linetype = "solid", size = 0.8) +
  scale_y_continuous(
...
```{r plot_dendermonde, fig.height=3, fig.width=8}
tidal_data %>% 
...
```{r plot_combined, fig.height=8, fig.width=10}
# Create custom y-axis labels by combining distance and name
...
tidal_tracks <-
  ...
  geom_ridgeline(
    ...
    aes(x = Timestamp, y = distance_from_sea, height = Value, group = distance_from_sea),
    scale = 1000,
    fill = "#9ecae1",
    color = "#2b8cbe"
  ) +
